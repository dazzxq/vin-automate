...
9. Redirect resolution (§4.5) achieves ≥90% success across ≥20 sampled URLs in `crawler-validation.md`.
...
| 8b | extract.py transient retry | Mock httpx returning network error 3× | retry attempted up to `MAX_RETRIES`, then `mark_failed`, `retry_count = MAX_RETRIES` |
